...
CORE_DB = ROOT / "outputs" / "step6_core_graphs" / "core_graph_full" / "step6_compound_pt_soc_core_full.sqlite"
...
def load_pt_soc() -> pd.DataFrame:
    ...
    with MEDDRA_MDHIER.open("r", encoding="latin-1") as f:
        for line in f:
            ...
            row = {"pt_code": pt_code, "pt_name": pt_name, "soc_code_primary": primary_soc_code, "soc_name_primary": soc_name}
...
def repair_db(db_path: Path, edge_tables: list[str]) -> dict:
    ...
    pt_df = pd.read_sql_query(f"SELECT pt_code, MIN(pt_name) AS pt_name FROM ({union_sql}) GROUP BY pt_code", conn)
    ...
    soc_df = pt_soc_df[["soc_name_primary", "soc_code_primary"]].drop_duplicates().rename(columns={"soc_name_primary": "soc_name", "soc_code_primary": "soc_code"}).reset_index(drop=True)
    for t in ["node_pt", "edge_pt_soc_primary", "node_soc"]:
        conn.execute(f"DROP TABLE IF EXISTS {t}")
    pt_df.to_sql("node_pt", conn, index=False, if_exists="replace")
    pt_soc_df[["pt_code", "soc_code_primary", "soc_name_primary"]].to_sql("edge_pt_soc_primary", conn, index=False, if_exists="replace")
    ...
    conn.commit()
    summary = {"db": str(db_path), "node_pt": int(conn.execute("select count(*) from node_pt").fetchone()[0]), "edge_pt_soc_primary": int(conn.execute("select count(*) from edge_pt_soc_primary").fetchone()[0]), "node_soc": int(conn.execute("select count(*) from node_soc").fetchone()[0])}
    conn.close()
    return summary
...
def write_summaries() -> None:
    ...
    core_node_summary = pd.DataFrame([("compound", core_conn.execute("select count(*) from node_compound").fetchone()[0]), ("pt", core_conn.execute("select count(*) from node_pt").fetchone()[0]), ("soc", core_conn.execute("select count(*) from node_soc").fetchone()[0])], columns=["node_type", "n_nodes"])
    core_edge_summary = pd.DataFrame([("compound_has_pt_ps", "compound", "pt", core_conn.execute("select count(*) from edge_compound_pt_ps").fetchone()[0]), ("compound_has_pt_ss", "compound", "pt", core_conn.execute("select count(*) from edge_compound_pt_ss").fetchone()[0]), ("pt_belongs_to_primary_soc", "pt", "soc", core_conn.execute("select count(*) from edge_pt_soc_primary").fetchone()[0])], columns=["edge_type", "src_type", "dst_type", "n_edges"])
