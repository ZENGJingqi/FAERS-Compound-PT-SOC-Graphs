...
SRC_DB = ROOT / "outputs" / "step14_heterograph" / "compound_pt_role_core_v3" / "step14_compound_pt_role_core_v3.sqlite"
BASE_OUT = ROOT / "outputs" / "step14_heterograph"
THRESHOLDS = [10, 20, 30]
...
def build_one(threshold: int) -> dict[str, object]:
    out_dir = BASE_OUT / f"compound_pt_role_core_ge{threshold}"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_db = out_dir / f"step14_compound_pt_role_core_ge{threshold}.sqlite"
    if out_db.exists():
        out_db.unlink()
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "这是在 `compound_pt_role_core_v3` 基础上裁剪得到的简化版主图。",
                "",
                "保留规则：",
